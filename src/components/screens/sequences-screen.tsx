...
function getStatusConfig(status: SequenceStatus) {
  switch (status) {
    case 'active':
      return { label: 'Active', color: tokens.confidence.high.value, bg: tokens.confidence.high.bg };
    case 'paused':
      return { label: 'Paused', color: tokens.confidence.medium.value, bg: tokens.confidence.medium.bg };
    case 'completed':
      ...
      {steps.slice(0, 4).map((step, i) => (
        <div key={step.id} className="flex items-center gap-1">
          <span
            ...
            style={{
              background: step.type === 'email' ? `${tokens.accent.primary}15` : step.type === 'task' ? `${tokens.domain.opportunity}15` : tokens.surfaceExtended,
              color: step.type === 'email' ? tokens.accent.primary : step.type === 'task' ? tokens.domain.opportunity : tokens.text.muted,
            }}
            title={step.label}
          >
          ...
          {i < Math.min(steps.length, 4) - 1 && (
            <span style={{ color: tokens.text.muted }} className="text-[10px]">→</span>
          )}
        </div>
      ))}
      {steps.length > 4 && (
        <span className="text-[10px]" style={{ color: tokens.text.muted }}>+{steps.length - 4}</span>
      )}
    </div>
  );
}
...
export default function Sequences() {
  ...
  const stats = useMemo(() => {
    ...
    const avgOpen = withOpens.length > 0
      ? withOpens.reduce((sum, s) => sum + s.openRate, 0) / withOpens.length
      : 0;
    const avgReply = withOpens.length > 0
  ...
  const sortedData = useMemo(() => {
    ...
    sorted.sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case 'openRate': aVal = a.openRate; bVal = b.openRate; break;
        case 'replyRate': aVal = a.replyRate; bVal = b.replyRate; break;
        case 'sentCount': aVal = a.sentCount; bVal = b.sentCount; break;
        case 'lastModified':
  ...
  const updateStep = useCallback((id: string, field: keyof SequenceStep, value: string | number) => {
      ...
      render: (_: unknown, row: Record<string, unknown>) => {
        ...
        return (
          <div className="min-w-[180px]">
            <p className="text-sm font-semibold" style={{ color: tokens.text.primary }}>{seq.name}</p>
            <p className="text-xs mt-0.5" style={{ color: tokens.text.muted }}>{seq.steps.length} steps</p>
          </div>
        );
      },
    },
      ...
      render: (value: unknown) => {
        ...
        return (
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium" style={{ color: rate > 60 ? tokens.confidence.high.value : rate > 40 ? tokens.confidence.medium.value : tokens.text.secondary }}>
              {rate > 0 ? `${rate}%` : '—'}
            </span>
          </div>
        );
      ...
      render: (value: unknown) => {
        ...
        return (
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium" style={{ color: rate > 15 ? tokens.confidence.high.value : rate > 8 ? tokens.confidence.medium.value : tokens.text.secondary }}>
              {rate > 0 ? `${rate}%` : '—'}
            </span>
          </div>
        );
      ...
      render: (value: unknown) => {
        ...
        return (
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium" style={{ color: tokens.text.primary }}>{count.toLocaleString()}</span>
          </div>
        );
      },
    },
        ...
        return (
          <button
            ...
            style={{
              ...
              color: seq.status === 'active' ? tokens.confidence.medium.value : tokens.confidence.high.value,
              background: seq.status === 'active' ? tokens.confidence.medium.bg : tokens.confidence.high.bg,
            }}
          >
  ...
  const tableData = useMemo(() => sortedData.map(s => ({ ...s })) as Record<string, unknown>[], [sortedData]);
  ...
  return (
    <div className="p-6 flex flex-col gap-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard icon={Zap} label="Active Sequences" value={stats.active} accent={tokens.confidence.high.value} />
          <StatCard icon={Send} label="Total Emails Sent" value={stats.totalSent.toLocaleString()} accent={tokens.accent.primary} />
          <StatCard icon={Eye} label="Avg Open Rate" value={`${stats.avgOpen.toFixed(1)}%`} accent={tokens.domain.reasoning} />
          <StatCard icon={MessageSquareReply} label="Avg Reply Rate" value={`${stats.avgReply.toFixed(1)}%`} accent={tokens.domain.opportunity} />
        </div>
      )}
          ...
          >
            <div className="flex items-center justify-between p-5" style={{ borderBottom: `1px solid ${tokens.border.default}` }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-semibold" style={{ color: tokens.text.primary }}>New Sequence</h2>
                  <p className="text-xs" style={{ color: tokens.text.muted }}>Define your outreach steps</p>
                </div>
              </div>
              <button onClick={() => setShowCreateModal(false)} className="p-1.5 rounded-lg transition-colors" style={{ color: tokens.text.muted }}>
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-5 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium" style={{ color: tokens.text.secondary }}>Sequence Name</label>
                <input
                  type="text"
                  value={newName}
              ...
              {/* Subject Template */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium" style={{ color: tokens.text.secondary }}>Subject Template</label>
                <input
                  type="text"
                  value={newSubject}
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-medium" style={{ color: tokens.text.secondary }}>Sequence Steps</label>
                  <button
                    onClick={addStep}
                    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium transition-colors"
                    style={{ color: tokens.accent.primary, border: `1px solid ${tokens.accent.primary}30` }}
                  >
                ...
                <div className="flex flex-col gap-2">
                  {newSteps.map((step, idx) => (
                    <div key={step.id} className="flex items-center gap-2 p-3 rounded-lg" style={{ background: tokens.surfaceExtended, border: `1px solid ${tokens.border.default}` }}>
                      <span className="flex items-center justify-center w-6 h-6 rounded text-xs font-bold shrink-0" style={{ background: `${tokens.accent.primary}15`, color: tokens.accent.primary }}>{idx + 1}</span>
                      <select
                        value={step.type}
                        onChange={(e) => updateStep(step.id, 'type', e.target.value)}
                        className="px-2 py-1 rounded-md text-xs outline-none"
                        style={{ background: tokens.surface.card, border: `1px solid ${tokens.border.default}`, color: tokens.text.primary }}
                      >
                      ...
                      {step.type === 'wait' && (
                        <div className="flex items-center gap-1">
                          <input
                            ...
                            style={{ background: tokens.surface.card, border: `1px solid ${tokens.border.default}`, color: tokens.text.primary }}
                          />
                          <span className="text-[10px]" style={{ color: tokens.text.muted }}>days</span>
                        </div>
                      )}
                      {newSteps.length > 1 && (
                        <button onClick={() => removeStep(step.id)} className="p-1 rounded transition-colors" style={{ color: tokens.text.muted }}>
