...
// Fallback queue data used when API is unavailable
const FALLBACK_QUEUE_DATA = [
  {
    id: 1,
    company: 'Stripe Inc.',
    domain: 'stripe.com',
...
export function ActivationWorkspace() {
  ...
  const [queueData, setQueueData] = useState(FALLBACK_QUEUE_DATA);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Fetch activation queue from signals API
  useEffect(() => {
    let cancelled = false;
    async function fetchData() {
      try {
        ...
        if (!res.error && res.data?.data?.length > 0) {
          const mapped = (res.data.data as any[]).map((o, i) => ({
            id: o.id || i + 1,
            company: o.name,
            domain: o.domain || '',
            industry: o.industry || '',
            status: (o.trackingStatus === 'active'
              ...
              : o.trackingStatus === 'completed'
                ? 'Completed'
                : 'Pending') as 'Active' | 'Completed' | 'Pending' | 'Failed',
            progress: o.intelligenceScore ?? 0,
          }));
          setQueueData(mapped);
        }
      } catch (err) {
        if (!cancelled)
          setError(err instanceof Error ? err.message : 'Failed to load activation queue');
      } finally {
            ...
            <div className="space-y-0">
              {TIMELINE_DATA.map((item, i) => (
                <motion.div
                  ...
                  style={{
                    borderBottom:
                      i < TIMELINE_DATA.length - 1 ? '1px solid var(--ios-border)' : 'none',
                  }}
                >
                  <span
