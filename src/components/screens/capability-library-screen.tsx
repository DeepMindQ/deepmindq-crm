        ...
        <div className={cn('flex-1 flex flex-col min-w-0 transition-all', selectedDoc ? 'mr-0 lg:mr-[560px]' : '')}>
            ...
            {coverage.total > 0 && (
              ...
              >
                <div className="flex items-center gap-2 text-xs text-zinc-400">
                  ...
                  <span className="text-zinc-200 font-semibold">{coverage.total}</span> snippets from{' '}
                  <span className="text-zinc-200 font-semibold">{documents.length}</span> documents
                </div>
                ...
                {Object.entries(TYPE_CONFIG).map(([key, cfg]) => (
                  coverage.byType[key] ? (
                    <span key={key} className="flex items-center gap-1 text-xs">
                      ...
                      <span className="text-zinc-200 font-medium">{coverage.byType[key]}</span>
                    </span>
                  ) : null
                ))}
                <Separator orientation="vertical" className="h-3 bg-zinc-800" />
                ...
                <AnimatedCard hover={false} className="!border-amber-500/20 !bg-amber-500/[0.03] mb-4">
                  <div className="p-4 space-y-3">
                    <div className="flex items-center gap-2 text-sm font-semibold text-amber-300">
                      ...
                      <span className="text-[10px] text-zinc-500 font-normal ml-2">Test what the AI will retrieve for a given context</span>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div className="relative">
                        ...
                        <Input
                          ...
                          className="pl-9 h-9 bg-zinc-900/80 border-zinc-700 text-zinc-200 text-xs"
                          value={engineQuery}
                      ...
                      <Input
                        ...
                        className="h-9 bg-zinc-900/80 border-zinc-700 text-zinc-200 text-xs"
                          ...
                          engineResults.results.map((r, i) => {
                            ...
                            return (
                              <motion.div
                                ...
                                className="rounded-lg border border-zinc-800/60 bg-zinc-900/60 p-3"
                              >
                                      ...
                                      {r.industries.length > 0 && (
                                        <div className="flex gap-1 mt-1.5">
                                          {r.industries.slice(0, 3).map(ind => (
                                            <span key={ind} className="text-[10px] text-zinc-400 bg-zinc-800 px-1.5 py-0.5 rounded">{ind}</span>
                                          ))}
                                        </div>
                                      )}
                                    </div>
          ...
          {loading ? (
            <div className="flex-1 flex items-center justify-center">
              <RefreshCw className="size-6 text-zinc-600 animate-spin" />
            </div>
          ) : documents.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
              <div className="size-20 rounded-2xl bg-zinc-800/50 flex items-center justify-center mb-4">
                <Database className="size-9 text-zinc-600" />
              </div>
              <h3 className="text-sm font-medium text-zinc-300 mb-1">No knowledge base yet</h3>
                  ...
                  {Object.entries(TYPE_CONFIG).map(([key, cfg]) => (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[11px] text-zinc-400">{cfg.label}s</span>
                      </div>
              ...
              {coverage.byIndustry.length > 0 && (
                <div className="mb-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-semibold text-zinc-300">Industry Coverage</span>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {coverage.byIndustry.map(item => {
                      ...
                      return (
                        <span
                          key={item.industry}
                          className="text-[11px] text-zinc-300 bg-zinc-800/80 border border-zinc-700/40 px-2 py-1 rounded-md"
                        >
                          {item.industry}
                          <span className="text-zinc-500 ml-1.5">{item.count}</span>
                        </span>
                      )
                    })}
                  </div>
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              <span className="text-[10px] text-zinc-500">{doc.fileType}</span>
                              <span className="text-[10px] text-zinc-600">|</span>
                              <span className="text-[10px] text-amber-500/80">{docSnipCount} snippets extracted</span>
                              {doc.description && (
                                <>
                                  <span className="text-[10px] text-zinc-600">|</span>
                                  <span className="text-[10px] text-zinc-500 truncate max-w-[200px]">{doc.description}</span>
                                </>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          <Badge variant="outline" className="text-[10px] text-zinc-500 border-zinc-700/50 h-5">
                        ...
                        {isExpanded && (
                          ...
                          >
                            <div className="px-4 pb-4 space-y-2 border-t border-zinc-800/40 pt-3">
                              {docSnips.length === 0 ? (
                                <p className="text-xs text-zinc-500 text-center py-4">No snippets extracted</p>
                              ) : (
                                docSnips.map((snip, i) => {
                                    ...
                                    >
                                      <div className="flex items-start gap-2">
                                        ...
                                        <div className="min-w-0 flex-1">
                                          <div className="flex items-center gap-2">
                                            <p className="text-xs font-medium text-zinc-200">{snip.title}</p>
                                            <Badge className={cn('text-[9px] h-4 border', cfg.border, cfg.bg, cfg.color)} variant="outline">
                                              {cfg.label}
                                            </Badge>
                                          </div>
                                          <p className="text-[11px] text-zinc-400 leading-relaxed mt-1 line-clamp-3">{snip.content}</p>
                                          {snip.industries.length > 0 && (
                                            <div className="flex gap-1 mt-1.5">
                                              {snip.industries.map(ind => (
                                                <span key={ind} className="text-[9px] text-zinc-500 bg-zinc-800 px-1.5 py-0.5 rounded">{ind}</span>
                                              ))}
                                            </div>
                                          )}
                                        </div>
              ...
              {filteredSnippets.length > 0 && (
                <div className="space-y-3 pb-6">
                  <div className="flex items-center gap-2 text-xs font-semibold text-zinc-400 uppercase tracking-wider">
                    ...
                    All Snippets
                    <span className="text-zinc-600 font-normal normal-case">({filteredSnippets.length})</span>
                  </div>
                    ...
                    {filteredSnippets.slice(0, 40).map(snip => {
                      ...
                      return (
                        <StaggerItem key={snip.id}>
                          <motion.div
                            className="rounded-xl border border-zinc-800/60 bg-zinc-900/40 p-4 cursor-pointer hover:border-amber-500/20 transition-colors"
                            whileHover={{ y: -2 }}
                          ...
                          >
                            ...
                            <h3 className="text-sm font-medium text-zinc-200 leading-snug mb-1.5">{snip.title}</h3>
                            <p className="text-[11px] text-zinc-400 leading-relaxed line-clamp-3">{snip.content}</p>
                            <div className="flex items-center gap-2 mt-2.5 flex-wrap">
                              {snip.industries.slice(0, 3).map(ind => (
                                <span key={ind} className="text-[10px] text-zinc-400 bg-zinc-800 px-1.5 py-0.5 rounded">{ind}</span>
                              ))}
                              {snip.documentTitle && (
                                <span className="text-[10px] text-zinc-600 ml-auto truncate max-w-[150px]">
          ...
          {selectedSnippet && (
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800/60 flex-shrink-0">
                ...
                <button onClick={() => setSelectedSnippet(null)} className="p-1.5 rounded-md text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800 transition-colors">
                  <X className="size-4" />
                </button>
              </div>
                ...
                <div className="px-6 py-5 space-y-5">
                  <div>
                    ...
                    {selectedSnippet.documentTitle && (
                      <p className="text-[11px] text-zinc-500 mt-1">Source: {selectedSnippet.documentTitle}</p>
                    )}
                  </div>
                  <div>
                    <h4 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Content</h4>
                    <div className="text-sm text-zinc-300 leading-relaxed bg-zinc-900/50 rounded-lg p-4 border border-zinc-800/50 whitespace-pre-wrap">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {selectedSnippet.industries.map(ind => (
                          <Badge key={ind} variant="secondary" className="text-[11px] bg-zinc-800 text-zinc-300 border border-zinc-700/50">{ind}</Badge>
                        ))}
                      </div>
                    </div>
                  )}
      ...
      <Dialog open={showUpload} onOpenChange={setShowUpload}>
        <DialogContent className="bg-zinc-900 border-zinc-800 max-w-lg">
          ...
          <div className="space-y-4 py-2">
            <div>
              <Label className="text-xs text-zinc-400 mb-1.5 block">Document Title</Label>
              <Input
            ...
            <div>
              <Label className="text-xs text-zinc-400 mb-1.5 block">File (.txt or .md)</Label>
              <div
                className={cn(
                  'border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition-colors',
                  uploadFile ? 'border-amber-500/40 bg-amber-500/5' : 'border-zinc-700 hover:border-zinc-600'
                )}
              ...
              >
                ...
                {uploadFile ? (
                  <div className="flex items-center justify-center gap-2 text-sm text-amber-300">
                    ...
                    {uploadFile.name}
                    <button onClick={(e) => { e.stopPropagation(); setUploadFile(null) }} className="text-zinc-500 hover:text-zinc-300">
                      <X className="size-3.5" />
                    </button>
                  </div>
                ) : (
                  <>
                    <FileUp className="size-8 text-zinc-600 mx-auto mb-2" />
                    <p className="text-xs text-zinc-400">Click to select a .txt or .md file</p>
                    <p className="text-[10px] text-zinc-600 mt-1">Max 10MB</p>
                  </>
                )}
              </div>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowUpload(false)} className="border-zinc-700 text-zinc-300 text-xs">
              Cancel
            </Button>
            <Button
