...
import { X, Mail, MailOpen, Reply, Paperclip, ArrowLeftRight } from 'lucide-react';
...
export default function Replies() {
  ...
  const openReply = useCallback((reply: EmailReply) => {
    ...
    setReplies((prev) =>
      prev.map((r) =>
        r.id === reply.id && r.status === 'unread' ? { ...r, status: 'read' as ReplyStatus } : r,
      ),
    );
  }, []);
        ...
        render: (value: unknown, row: Record<string, unknown>) => {
          ...
          return (
            <div className="min-w-[160px]">
              <p
                className="text-sm"
                style={{ color: tokens.text.primary, fontWeight: isUnread ? 600 : 400 }}
              >
                {reply.from}
              </p>
  ...
  const tableData = useMemo(
    () => filteredData.map((r) => ({ ...r })) as Record<string, unknown>[],
    [filteredData],
  );
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs" style={{ color: tokens.text.muted }}>
                    {formatDateTime(viewingReply.receivedAt)}
                  </span>
