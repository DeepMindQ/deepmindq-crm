...
const MOCK_TASKS: Task[] = [
  ...
  {
    id: 't-2',
    name: 'Follow up with TechStart demo',
  ...
  {
    id: 't-3',
    name: 'Prepare Q1 pipeline review',
    assignedTo: 'Sarah Chen',
    dueDate: tomorrow,
    priority: 'high',
    status: 'todo',
  ...
  {
    id: 't-4',
    name: 'Update CRM for Vertex Solutions',
  ...
  {
    id: 't-5',
    name: 'Research competitor positioning',
    assignedTo: 'Emily Park',
    dueDate: nextWeek,
    priority: 'low',
    status: 'todo',
    relatedCompany: '—',
  ...
  {
    id: 't-6',
    name: 'Schedule call with DataFlow',
    assignedTo: 'James Wilson',
    dueDate: twoDaysAgo,
    priority: 'high',
    status: 'done',
  ...
  {
    id: 't-7',
    name: 'Draft intro email for HealthFirst',
    assignedTo: 'Sarah Chen',
    dueDate: today,
    priority: 'medium',
    status: 'todo',
  ...
  {
    id: 't-8',
    name: 'Review intelligence report for FinEdge',
    assignedTo: 'Mike Rodriguez',
    dueDate: tomorrow,
    priority: 'critical',
    status: 'in_progress',
  ...
  {
    id: 't-9',
    name: 'Add contacts for CloudNova',
    assignedTo: 'Emily Park',
    dueDate: twoDaysAgo,
    priority: 'medium',
    status: 'done',
  ...
  {
    id: 't-10',
    name: 'Send case study to NexGen',
    assignedTo: 'James Wilson',
    dueDate: today,
    priority: 'low',
    status: 'todo',
...
function getPriorityConfig(priority: TaskPriority) {
  switch (priority) {
    case 'critical':
      return {
        label: 'Critical',
        color: tokens.confidence.critical.value,
        bg: tokens.confidence.critical.bg,
      };
    case 'high':
...
export default function Tasks() {
  ...
  const tableData = useMemo(
    () => filteredData.map((t) => ({ ...t })) as Record<string, unknown>[],
    [filteredData],
  );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: tokens.text.secondary }}>
            Manage your sales tasks and follow-ups
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            label="Overdue"
            value={stats.overdue}
            accent={tokens.confidence.low.value}
            sub="Needs attention"
          />
          <StatCard
            icon={CalendarClock}
          ...
          <StatCard
            ...
            sub={`${Math.round((stats.completedThisWeek / stats.total) * 100)}% completion rate`}
          />
        </div>
      )}
