  ...
  if (compact) {
    return (
      <div className={cn('flex items-center gap-2', className)}>
        ...
        <span className="text-[10px] font-mono tabular-nums shrink-0" style={{ color: scoreColor }}>
          {overallScore}%
        </span>
        <span className="text-[10px] shrink-0" style={{ color: tokens.text.muted }}>
            ...
            return (
              <div key={field.name} className="flex items-center gap-3 px-4 py-2">
                <FreshIcon
                  ...
                  style={{ color: field.complete ? freshConfig.color : tokens.text.muted }}
                />
                <span
                  className={cn('text-[11px] flex-1 truncate', !field.complete && 'italic')}
                  style={{ color: field.complete ? tokens.text.primary : tokens.text.muted }}
                >
                  {field.name}
                </span>
                {field.value && (
                ...
                {field.lastUpdated && (
                  <span className="text-[9px] shrink-0" style={{ color: freshConfig.color }}>
                    {timeAgo(field.lastUpdated)}
                  </span>
                )}
                {field.source && (
                  <span
