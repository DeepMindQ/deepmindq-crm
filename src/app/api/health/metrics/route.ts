...
export async function GET() {
  ...
  // ── Platform liveness ───────────────────────────────────────
  lines.push(metricHelpTypeValue('deepmindq_up', 'Whether the DeepMindQ platform is up (1 = up)', 'gauge', 1));
  lines.push('');

  lines.push(metricHelpTypeValue('deepmindq_uptime_seconds', 'Process uptime in seconds', 'gauge', process.uptime()));
  lines.push('');
  ...
  try {
    ...
    lines.push(metricHelpTypeValue('deepmindq_memory_usage_bytes', 'Process RSS memory usage in bytes', 'gauge', memBytes));
    lines.push('');
    ...
    lines.push(metricHelpTypeValue('deepmindq_gc_collections_total', 'Total GC collections since startup', 'counter', gcCount));
    lines.push('');
  } catch (err) {
    lines.push(`# ERROR collecting system metrics: ${err instanceof Error ? err.message : String(err)}`);
  ...
  try {
    ...
    lines.push(metricHelpTypeValue(
    ...
    const errorCount = apiMetrics.totalRequests > 0
    ...
    lines.push(`# ERROR collecting HTTP metrics: ${err instanceof Error ? err.message : String(err)}`);
  ...
  try {
    ...
    lines.push(`# ERROR collecting DB metrics: ${err instanceof Error ? err.message : String(err)}`);
  ...
  try {
    ...
    lines.push(`# ERROR collecting AI provider metrics: ${err instanceof Error ? err.message : String(err)}`);
  ...
  try {
    ...
    lines.push(metricHelpTypeValue('deepmindq_event_loop_lag_ms', 'Event loop lag in milliseconds', 'gauge', eventLoopLag));
    lines.push('');
  } catch { /* ignore */ }
  ...
  try {
    ...
    lines.push(metricHelpTypeValue('deepmindq_db_queries_per_second', 'Database queries per second', 'gauge', safeNumber(dbPerf.queriesPerSecond)));
    lines.push('');
    lines.push(metricHelpTypeValue('deepmindq_db_query_latency_p50_ms', 'Database query P50 latency in milliseconds', 'gauge', safeNumber(dbPerf.p50LatencyMs)));
    lines.push('');
    lines.push(metricHelpTypeValue('deepmindq_db_query_latency_p99_ms', 'Database query P99 latency in milliseconds', 'gauge', safeNumber(dbPerf.p99LatencyMs)));
    lines.push('');
    lines.push(metricHelpTypeValue('deepmindq_db_slow_queries_total', 'Total slow database queries in current window', 'gauge', safeNumber(dbPerf.slowQueryCount)));
    lines.push('');
  } catch { /* ignore */ }
  ...
  try {
    ...
    lines.push(metricHelpTypeValue('deepmindq_heap_usage_percent', 'Heap usage percentage', 'gauge', safeNumber(memHealth.heapUsagePercentage)));
    lines.push('');
    lines.push(metricHelpTypeValue('deepmindq_memory_growth_rate_mb_per_min', 'Memory growth rate in MB per minute', 'gauge', safeNumber(memHealth.memoryGrowthRateMbPerMin)));
    lines.push('');
    lines.push(metricHelpTypeValue('deepmindq_ai_active_contexts', 'Number of active AI processing contexts', 'gauge', safeNumber(memHealth.aiContextStats.activeContexts)));
    lines.push('');
  } catch { /* ignore */ }
  ...
  try {
    ...
    lines.push(metricHelpTypeValue('deepmindq_incidents_active', 'Number of active incidents', 'gauge', safeNumber(summary.active)));
    lines.push('');
    lines.push(metricHelpTypeValue('deepmindq_incidents_sla_breached', 'Number of incidents with breached SLA', 'gauge', safeNumber(summary.slaBreached)));
    lines.push('');
  } catch { /* ignore */ }

  // ── SLA Breach Metrics (from sla-monitor.ts) ──
  try {
    const { getRouteSLAReport, getRawRouteStats, getSLAThreshold } = await import('@/lib/sla-monitor')
  ...
  try {
    ...
    if (!agg['system.memory.rss']) {
      lines.push(metricHelpTypeValue('deepmindq_memory_usage_bytes', 'Process RSS memory usage in bytes', 'gauge', mem.rss));
      lines.push('');
    }
  } catch {
    // ignore
  ...
  try {
    ...
    lines.push(metricHelpTypeValue('deepmindq_intelligence_cache_memory_entries', 'Number of entries in the in-memory intelligence cache', 'gauge', cacheStats.memoryEntries));
    lines.push('');
    lines.push(metricHelpTypeValue('deepmindq_intelligence_cache_memory_max_entries', 'Maximum capacity of the in-memory intelligence cache', 'gauge', cacheStats.memoryMaxEntries));
    lines.push('');
    lines.push(metricHelpTypeValue('deepmindq_intelligence_cache_redis_available', 'Whether Redis is available for intelligence caching (1 = available)', 'gauge', cacheStats.redisAvailable ? 1 : 0));
    lines.push('');
  } catch {
