...
export async function GET() {
  try {
    ...
    const signalsLast30Days = await db.companySignal.count({
      where: { createdAt: { gte: thirtyDaysAgo } },
    });
    ...
    const signalCoverage = await db.company.count() > 0
      ? parseFloat(((companiesWithSignals.length / await db.company.count()) * 100).toFixed(1))
      : 0;
    ...
    const enrichedCompanies = await db.company.count({
      where: { intelligenceScore: { gte: 3 } },
    });
    const enrichmentRate = companies > 0 ? parseFloat(((enrichedCompanies / companies) * 100).toFixed(1)) : 0;
    ...
    const companiesWithoutIndustry = await db.company.count({
      where: { industry: null },
    });
    ...
    const dataHealthScore = Math.round(
      ...
      (companies > 0 ? ((companies - companiesWithoutIndustry) / companies) * 100 * 0.1 : 0)
    );
