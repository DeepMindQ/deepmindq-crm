...
function getCriterionColor(field: string): string {
  switch (field) {
    case 'industry': return tokens.domain.reasoning;
    case 'employeeCount':
    case 'fundingStage': return tokens.accent.primary;
    case 'intelligenceScore': return tokens.domain.opportunity;
    case 'signalCount': return tokens.confidence.medium.value;
    case 'trackingStatus': return tokens.domain.risk;
...
export default function Segments() {
  ...
  const updateCriterion = useCallback((idx: number, field: keyof SegmentCriterion, value: string) => {
  ...
  return (
    <div className="p-6 flex flex-col gap-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <StatCard icon={Layers} label="Total Segments" value={stats.total} accent={tokens.accent.primary} />
          <StatCard icon={Building2} label="Total Companies" value={stats.totalCompanies.toLocaleString()} accent={tokens.domain.reasoning} sub="Across all segments" />
          <StatCard icon={Target} label="Avg Match Score" value={stats.avgScore} accent={tokens.domain.opportunity} sub="Weighted by company count" />
        </div>
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="rounded-xl p-5" style={{ background: tokens.surface.card, border: `1px solid ${tokens.border.default}` }}>
              <Skeleton className="h-5 w-32 mb-2 rounded" style={{ background: tokens.border.default }} />
              <Skeleton className="h-3 w-full mb-1 rounded" style={{ background: tokens.border.default }} />
              <Skeleton className="h-3 w-3/4 mb-4 rounded" style={{ background: tokens.border.default }} />
              <div className="flex gap-2 mb-4">
                <Skeleton className="h-6 w-16 rounded-full" style={{ background: tokens.border.default }} />
                <Skeleton className="h-6 w-16 rounded-full" style={{ background: tokens.border.default }} />
              </div>
              <div className="flex gap-4">
                <Skeleton className="h-10 w-20 rounded-lg" style={{ background: tokens.border.default }} />
                <Skeleton className="h-10 w-20 rounded-lg" style={{ background: tokens.border.default }} />
              </div>
            </div>
          ))}
      ...
      {!loading && segments.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 px-6">
          ...
          <h3 className="text-base font-semibold mb-1.5" style={{ color: tokens.text.primary }}>No segments yet</h3>
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h3 className="text-sm font-semibold truncate" style={{ color: tokens.text.primary }}>{seg.name}</h3>
                      <p className="text-xs mt-0.5" style={{ color: tokens.text.muted }}>Updated {formatRelativeDate(seg.lastUpdated)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 rounded-lg" style={{ background: tokens.surfaceExtended }}>
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <span className="text-[10px] font-medium" style={{ color: tokens.text.muted }}>Companies</span>
                    </div>
                    <p className="text-lg font-bold" style={{ color: tokens.text.primary }}>{seg.companyCount}</p>
                  </div>
                  <div className="p-3 rounded-lg" style={{ background: tokens.surfaceExtended }}>
                    <div className="flex items-center gap-1.5 mb-1">
                      <Target className="w-3.5 h-3.5" style={{ color: tokens.text.muted }} />
                      <span className="text-[10px] font-medium" style={{ color: tokens.text.muted }}>Avg Score</span>
                    </div>
                    <p className="text-lg font-bold" style={{ color: scoreColor.color }}>{seg.avgIntelScore}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {seg.criteria.map((c, i) => (
                    <span
                      ...
                      style={{ background: `${getCriterionColor(c.field)}15`, color: getCriterionColor(c.field) }}
                    >
          ...
          >
            <div className="flex items-center justify-between p-5" style={{ borderBottom: `1px solid ${tokens.border.default}` }}>
              <div className="flex items-center gap-3">
                <div className="flex items-center justify-center w-9 h-9 rounded-lg" style={{ background: `${tokens.domain.reasoning}15`, color: tokens.domain.reasoning }}>
                  <PieChart className="w-5 h-5" />
                </div>
                <div>
                  <h2 className="text-base font-semibold" style={{ color: tokens.text.primary }}>New Segment</h2>
                  <p className="text-xs" style={{ color: tokens.text.muted }}>Define criteria for your customer segment</p>
                </div>
              </div>
            ...
            <div className="p-5 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium" style={{ color: tokens.text.secondary }}>Segment Name</label>
                <input
                  type="text"
                  value={newName}
              ...
              {/* Description */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium" style={{ color: tokens.text.secondary }}>Description</label>
                <textarea
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-medium" style={{ color: tokens.text.secondary }}>Criteria</label>
                  <button
                ...
                <div className="flex flex-col gap-2">
                  {newCriteria.map((c, idx) => (
                    <div key={idx} className="flex items-center gap-2 p-3 rounded-lg" style={{ background: tokens.surfaceExtended, border: `1px solid ${tokens.border.default}` }}>
                      <select
                        value={c.field}
                      ...
                      {newCriteria.length > 1 && (
                        <button onClick={() => removeCriterion(idx)} className="p-1 rounded transition-colors" style={{ color: tokens.text.muted }}>
