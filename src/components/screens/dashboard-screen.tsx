...
import {
  ...
  Sparkles, Brain, Layers, RefreshCw,
...
export default function DashboardScreen({ navigateTo }: { navigateTo?: (screen: string, companyId?: string) => void }) {
  ...
  const { data: activity = [], isLoading: activityLoading, isError: activityError, refetch: refetchActivity } = useQuery<AuditEntry[]>({
  ...
  const { data: rawCompanies, isLoading: companiesLoading, isError: companiesError, refetch: refetchCompanies } = useQuery<{ companies?: RawCompany[]; data?: RawCompany[] } | undefined>({
  ...
  const { data: rawSegments, isLoading: segmentsLoading, isError: segmentsError, refetch: refetchSegments } = useQuery<RawSegment[] | { data: RawSegment[] } | undefined>({
  ...
  if (allZero && dashData) return (
    <div className="flex flex-col items-center justify-center py-20 text-center max-w-md mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-center gap-3 mt-6">
        <button onClick={() => nav('import')} className="flex items-center gap-2 text-xs font-semibold px-4 py-2.5 rounded-lg transition-colors" style={{ background: gold, color: 'var(--dmq-black)' }}>
  ...
  if (dashData && !dashData.contactsByStatus) return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <button onClick={() => nav('import')} className="mt-3 flex items-center gap-1.5 text-xs font-medium px-3 py-2.5 rounded-lg hover:bg-gray-100 transition-colors min-h-[44px]" style={{ color: gold }}>
    ...
    <div role="main" aria-label="Executive Dashboard" className="max-h-[calc(100vh-200px)] overflow-y-auto space-y-5 pr-1">
      ...
      {statsFailed && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          ...
          <button onClick={() => refetchStats()} className="flex items-center gap-1 text-xs font-medium px-2.5 py-2.5 rounded-lg hover:bg-red-50 transition-colors" style={{ color: colors.red }}>
      ...
      {briefingLoading ? (
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center gap-3 px-5 py-3.5">
            ...
            <button onClick={() => refetchBriefing()} className="ml-auto flex items-center gap-1.5 text-xs font-medium px-3 py-2.5 rounded-lg hover:bg-gray-100 transition-colors min-h-[44px]" style={{ color: gold }}>
              <RefreshCw className="w-3 h-3" /> Retry
            </button>
          </div>
        </motion.div>
          ...
          <div className="flex-1 px-5 pb-4 max-h-80 overflow-y-auto custom-scrollbar">
            {companiesError ? (
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <button onClick={() => refetchCompanies()} className="mt-2 text-xs font-medium px-3 py-2.5 rounded-lg hover:bg-gray-100 transition-colors" style={{ color: gold }}><RefreshCw className="w-3 h-3 inline mr-1" />Retry</button>
              </div>
            ) : topCompanies.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-center">
          ...
          <div className="px-5 pb-5">
            {activityError ? (
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <button onClick={() => refetchActivity()} className="mt-2 text-xs font-medium px-3 py-2.5 rounded-lg hover:bg-gray-100 transition-colors" style={{ color: gold }}><RefreshCw className="w-3 h-3 inline mr-1" />Retry</button>
              </div>
            ) : activity.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-center">
                <motion.div animate={{ scale: [1, 1.08, 1] }} transition={{ duration: 2.5, repeat: Infinity, ease: 'easeInOut' }}
          ...
          <div className="px-5 pb-5 space-y-3 pt-2">
            {segmentsError ? (
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <button onClick={() => refetchSegments()} className="mt-2 text-xs font-medium px-3 py-2.5 rounded-lg hover:bg-gray-100 transition-colors" style={{ color: gold }}><RefreshCw className="w-3 h-3 inline mr-1" />Retry</button>
              </div>
            ) : segments.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-center">
