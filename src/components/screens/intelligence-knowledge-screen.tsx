...
function getTypeColor(type: string) {
  switch (type) {
    case 'Company': return { bg: tokens.accent.subtle, color: tokens.accent.primary };
    case 'Person': return { bg: tokens.domain.bg, color: tokens.domain.value };
    case 'Industry': return { bg: tokens.confidence.high.bg, color: tokens.confidence.high.value };
    case 'Technology': return { bg: tokens.gold.bgMedium, color: tokens.gold.dark };
    default: return { bg: tokens.neutral['100'], color: tokens.text.secondary };
  }
}
...
export default function IntelligenceKnowledge() {
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold" style={{ color: tokens.text.primary }}>Knowledge Graph</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Nodes', value: '24,891', icon: Network, color: tokens.accent.primary, bg: tokens.accent.subtle },
          { label: 'Total Edges', value: '187,342', icon: GitBranch, color: tokens.domain.value, bg: tokens.domain.bg },
          { label: 'Avg Connections', value: '15.1', icon: Network, color: tokens.confidence.high.value, bg: tokens.confidence.high.bg },
          { label: 'Graph Coverage', value: '94.2%', icon: GitBranch, color: tokens.gold.dark, bg: tokens.gold.bgMedium },
        ].map((stat) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <CardContent className="flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-1 max-h-96 overflow-y-auto">
                {filteredEntities.length === 0 ? (
                  <div className="py-8 text-center text-sm" style={{ color: tokens.text.muted }}>No entities found</div>
                ) : (
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          border: isSelected ? `1px solid ${tokens.accent.subtle}` : '1px solid transparent',
                        }}
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          <div className="size-2 rounded-full shrink-0" style={{ backgroundColor: typeStyle.color }} />
                          <div className="min-w-0">
                            <p className="text-sm font-medium truncate" style={{ color: tokens.text.primary }}>{entity.name}</p>
                            <p className="text-xs" style={{ color: tokens.text.muted }}>{entity.type}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-1 shrink-0">
                          <GitBranch className="size-3" style={{ color: tokens.text.muted }} />
                          <span className="text-xs font-mono" style={{ color: tokens.text.secondary }}>{entity.connections}</span>
                        </div>
                      </button>
                    );
                  })
        ...
        <div className="lg:col-span-2">
          <Card className="gap-4 py-4 h-full">
              ...
              {selectedEntity && selectedName ? (
                <div className="flex items-center justify-between">
                  <CardTitle className="text-sm font-semibold">
                    Connections for <span style={{ color: tokens.domain.value }}>{selectedName}</span>
                  </CardTitle>
            ...
            <CardContent>
              {!selectedEntity ? (
                <div className="flex flex-col items-center justify-center h-64 gap-3" style={{ color: tokens.text.muted }}>
                  <Network className="size-12 opacity-30" />
                  <p className="text-sm">Select an entity to view its connections</p>
                </div>
              ) : connections.length === 0 ? (
                <div className="flex items-center justify-center h-32" style={{ color: tokens.text.muted }}>
                  <p className="text-sm">No connections found for this entity.</p>
                </div>
              ) : (
                <div className="flex flex-col gap-3 max-h-96 overflow-y-auto">
                  <div className="rounded-lg p-4" style={{ backgroundColor: tokens.accent.ghost, border: `1px solid ${tokens.accent.subtle}` }}>
                    <div className="flex items-center gap-2 mb-2">
                      <div className="size-3 rounded-full" style={{ backgroundColor: getTypeColor(TOP_ENTITIES.find((e) => e.id === selectedEntity)?.type || '')?.color }} />
                      <span className="text-sm font-semibold" style={{ color: tokens.text.primary }}>{selectedName}</span>
                    </div>
                    <p className="text-xs" style={{ color: tokens.text.secondary }}>{connections.length} connections in knowledge graph</p>
                  </div>
                  {connections.map((conn, idx) => {
                    const typeStyle = getTypeColor(conn.type);
                    return (
                      <div key={idx} className="flex items-center gap-3 p-3 rounded-lg" style={{ border: `1px solid ${tokens.border.default}` }}>
                        <div className="size-2 rounded-full shrink-0" style={{ backgroundColor: typeStyle.color }} />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium" style={{ color: tokens.text.primary }}>{conn.name}</p>
                          <div className="flex items-center gap-2 mt-0.5">
                            <Badge className="text-[10px] px-1.5 py-0" style={{ backgroundColor: typeStyle.bg, color: typeStyle.color }}>{conn.type}</Badge>
                          </div>
                        </div>
                        <div className="flex items-center gap-1.5 shrink-0 px-3 py-1.5 rounded-full" style={{ backgroundColor: tokens.surface.secondary }}>
                          <span className="text-xs" style={{ color: tokens.text.muted }}>{conn.relation}</span>
