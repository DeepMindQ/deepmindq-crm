...
function OpportunityCard({ opp, onViewBrief }: { opp: OpportunityItem; onViewBrief: (id: string) => void }) {
  const renderCatIcon = () => {
    switch (opp.category) {
      case 'HOT_ACCOUNT': return <Flame className="w-3 h-3" aria-hidden="true" />;
      case 'WARM_ACCOUNT': return <Sun className="w-3 h-3" aria-hidden="true" />;
      case 'NURTURE': return <Sprout className="w-3 h-3" aria-hidden="true" />;
      case 'AT_RISK': return <AlertTriangle className="w-3 h-3" aria-hidden="true" />;
      default: return <Target className="w-3 h-3" aria-hidden="true" />;
    }
  };
  return (
    <div className="bg-white border border-border rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="p-5 space-y-4">
        <div className="flex items-start justify-between">
          ...
          <div className="flex flex-col items-end gap-1 shrink-0 ml-3"><span className={`inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wider ${categoryColor(opp.category)}`}>{renderCatIcon()}{categoryLabel(opp.category)}</span><div className="w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold bg-gradient-to-br from-gray-800 to-gray-900 text-white">{opp.score}</div></div>
        </div>
        <p className="text-sm text-foreground/80 leading-relaxed line-clamp-2">{opp.description}</p>
        <div className="flex flex-wrap gap-1.5">{opp.topSignals.slice(0, 4).map((s, i) => (<Badge key={i} variant="outline" className={`text-[11px] ${signalTypeColor(s.type)}`}>{s.type} {s.score}</Badge>))}{opp.signalCount > 4 && <Badge variant="secondary" className="text-[11px]">+{opp.signalCount - 4} more</Badge>}</div>
