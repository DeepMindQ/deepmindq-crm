...
const INDUSTRIES = ['SaaS', 'Fintech', 'Healthcare', 'E-commerce', 'Cybersecurity', 'AI/ML', 'EdTech', 'Logistics', 'Manufacturing', 'LegalTech'];
...
const mockAccounts = Array.from({ length: 15 }, (_, i) => {
  ...
  return {
    rank: i + 1,
    company: [
      'Acme Corp', 'Vertex Solutions', 'NovaTech AI', 'Stratos Inc', 'Helix Biotech',
      'Quantum Financial', 'Pinnacle Systems', 'Nexus Digital', 'Cirrus Cloud', 'Titan Enterprises',
      'Aurora Health', 'Apex Robotics', 'Meridian Labs', 'Zenith Data', 'Forge Analytics',
    ][i],
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="py-4 gap-4">
          <CardContent className="px-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm" style={{ color: tokens.text.secondary }}>Total Accounts</p>
              <p className="text-2xl font-bold" style={{ color: tokens.text.primary }}>{stats.total}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="py-4 gap-4">
          <CardContent className="px-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm" style={{ color: tokens.text.secondary }}>Gold Tier</p>
              <p className="text-2xl font-bold" style={{ color: tokens.text.primary }}>{stats.gold}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="py-4 gap-4">
          <CardContent className="px-4 flex items-center gap-3">
            <div className="rounded-lg p-2.5" style={{ backgroundColor: tokens.confidence.high.bg }}>
              <TrendingUp className="size-5" style={{ color: tokens.confidence.high.value }} />
            </div>
            <div>
              <p className="text-sm" style={{ color: tokens.text.secondary }}>Avg Score</p>
              <p className="text-2xl font-bold" style={{ color: tokens.text.primary }}>{stats.avg}</p>
            </div>
          </CardContent>
        </Card>
      </div>
              ...
              <TableBody>
                {filtered.map((account) => (
                  <TableRow key={account.rank} className="cursor-pointer">
                    <TableCell className="pl-6 font-mono font-bold" style={{ color: tokens.text.primary }}>
                      #{account.rank}
                    </TableCell>
                    <TableCell>
                      <span className="font-medium" style={{ color: tokens.text.primary }}>{account.company}</span>
                    </TableCell>
                    <TableCell style={{ color: tokens.text.secondary }}>{account.industry}</TableCell>
                    <TableCell><ScoreBar score={account.score} /></TableCell>
                    <TableCell style={{ color: tokens.text.secondary }}>{account.signals}</TableCell>
                    <TableCell className="font-mono" style={{ color: tokens.text.secondary }}>{account.revenue}</TableCell>
                    <TableCell><TierBadge tier={account.tier} /></TableCell>
                    <TableCell className="pr-6" style={{ color: tokens.text.muted }}>{account.lastUpdated}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
