...
const MOCK_ACTIVITY: ActivityItem[] = [
  { id: 'a1', action: 'Deployed v2.4.1 to production', user: 'CI/CD Pipeline', time: '2 min ago', type: 'success' },
  { id: 'a2', action: 'AI provider rate limit hit (OpenAI)', user: 'System', time: '8 min ago', type: 'warning' },
  { id: 'a3', action: 'New user onboarded: Acme Corp (12 seats)', user: 'Sarah Chen', time: '15 min ago', type: 'info' },
  { id: 'a4', action: 'Database backup completed successfully', user: 'System', time: '32 min ago', type: 'success' },
  { id: 'a5', action: 'Email delivery failure rate > 5%', user: 'System', time: '45 min ago', type: 'error' },
  { id: 'a6', action: 'Import job completed: 1,247 records', user: 'James Wilson', time: '1 hour ago', type: 'success' },
  { id: 'a7', action: 'Webhook endpoint updated for Salesforce', user: 'Maria Garcia', time: '1.5 hours ago', type: 'info' },
  { id: 'a8', action: 'Memory usage exceeded 80% threshold', user: 'System', time: '2 hours ago', type: 'warning' },
  { id: 'a9', action: 'SSO configuration updated for Okta', user: 'David Kim', time: '3 hours ago', type: 'info' },
  { id: 'a10', action: 'AI model switched to Claude 3.5', user: 'Admin', time: '4 hours ago', type: 'success' },
];
...
export default function Enterprise() {
  ...
  return (
    <div className="p-6 space-y-6" style={{ background: '#0a0e17', minHeight: '100%' }}>
      {/* ── Header ── */}
      <div>
        <h1 className="text-xl font-bold" style={{ color: textPrimary }}>Enterprise Overview</h1>
        <p className="text-sm mt-1" style={{ color: textSecondary }}>System health, activity, and admin quick access</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* ── System Status ── */}
        <div className="lg:col-span-1 rounded-xl p-5" style={{ background: bg, border: `1px solid ${border}`, boxShadow: elevation.sm }}>
          <h3 className="text-sm font-semibold mb-4" style={{ color: textPrimary }}>System Status</h3>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-medium" style={{ color: textPrimary }}>{sys.name}</p>
                      <p className="text-xs" style={{ color: textMuted }}>{sys.latency}</p>
                    </div>
                  </div>
                  <span
          ...
          <div className="mt-6 pt-4" style={{ borderTop: `1px solid ${border}` }}>
            <h4 className="text-xs font-semibold mb-3" style={{ color: textSecondary }}>Quick Links</h4>
            <div className="space-y-2">
              {[
                ...
                >
                  ...
                  <span className="text-xs font-medium" style={{ color: textPrimary }}>{link.label}</span>
        ...
        {/* ── Activity Feed ── */}
        <div className="lg:col-span-2 rounded-xl p-5" style={{ background: bg, border: `1px solid ${border}`, boxShadow: elevation.sm }}>
          <h3 className="text-sm font-semibold mb-4" style={{ color: textPrimary }}>Activity Feed</h3>
            ...
            {MOCK_ACTIVITY.map((item, idx) => {
              ...
              return (
                <div
                  ...
                  style={{ borderBottom: idx < MOCK_ACTIVITY.length - 1 ? `1px solid ${border}` : 'none' }}
                >
                  <div className="w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5" style={{ background: `${color}15` }}>
                    <Icon className="w-3.5 h-3.5" style={{ color }} />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-xs" style={{ color: textPrimary }}>{item.action}</p>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-xs" style={{ color: textMuted }}>{item.user}</span>
                      <span className="text-xs" style={{ color: textMuted }}>·</span>
                      <span className="text-xs" style={{ color: textMuted }}>{item.time}</span>
                    </div>
                  </div>
                </div>
              );
