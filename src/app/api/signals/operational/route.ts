...
/* ═══════════════════════════════════════════════════════════════
   Types — Operational contact-level signals (utility detection)
   ═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   GET /api/signals/operational — Detect signals from lead data
   ═══════════════════════════════════════════════════════════════ */
...
const startedAt = Date.now();
  ...
  try {
    ctx = utilityGuard(request, 'signals-operational');
  } catch (err) {
  ...
  try {
    ...
    try {
      const scoreSpikeContacts = await db.contact.findMany({
        where: { leadScore: { gte: 80 }, status: { in: ['imported', 'cleaned'] } },
        include: { company: { select: { normalizedName: true } } },
        take: 10,
        orderBy: { leadScore: 'desc' },
      });
    ...
    try {
      const staleContacts = await db.contact.findMany({
        where: { status: 'sent', lastContactedAt: { lte: sevenDaysAgo } },
    ...
    try {
      const riskyQueueContacts = await db.contact.findMany({
        where: {
          emailHealth: 'risky',
          drafts: { some: { queueItem: { status: { in: ['pending', 'scheduled'] } } } },
        },
        include: { company: { select: { normalizedName: true } } },
        take: 10,
      });
    ...
    try {
      const unassignedContacts = await db.contact.findMany({
        where: { leadScore: { gte: 70 }, assignedTo: null },
        include: { company: { select: { normalizedName: true } } },
        take: 10,
        orderBy: { leadScore: 'desc' },
      });
    ...
    try {
      const dropouts = await db.sequenceEnrollment.findMany({
        ...
        include: {
          contact: { select: { id: true, rawName: true, company: { select: { normalizedName: true } } } },
    ...
    }, 'signals-operational', Date.now() - startedAt);
  } catch (err) {
...
/* ═══════════════════════════════════════════════════════════════
   POST /api/signals/operational — Dismiss a signal (in-memory)
   ═══════════════════════════════════════════════════════════════ */
