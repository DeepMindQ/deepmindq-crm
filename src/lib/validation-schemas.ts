...
export const aiExperimentPatchSchema = z
  .object({
  ...
  .refine((d) => d.action || (d.metric && d.variantId && d.value !== undefined), {
    message:
      'Specify either "action" (start/pause/resume/complete) or metric recording fields (metric, variantId, value)',
  });
...
export const adminScoringSchema = z
  .object({
...
export const accountDataDeletionInitiateSchema = z
  .object({
    reason: z.string().max(500).optional(),
    scope: z
      .object({
...
export const crmSyncOptionsSchema = z
  .object({
...
export const duplicateMergePostSchema = z.object({
  ...
  strategy: z
    .enum(['keep_survivor', 'keep_duplicate', 'keep_most_recent'])
    .default('keep_survivor'),
...
export const aiEvaluationPostSchema = z
  .object({
    ...
    comparisonType: z
      .enum(['prompt_version', 'model', 'engine_version', 'configuration'])
      .optional(),
  })
  .refine(
    (d) => {
      ...
      if (d.action === 'compare')
        return !!d.labelA && !!d.labelB && Array.isArray(d.resultsA) && Array.isArray(d.resultsB);
      return false;
    },
...
export const genericIdBodySchema = z
  .object({
    id: z.string().min(1),
  })
  .passthrough();
