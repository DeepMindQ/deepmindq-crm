  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 pb-20">
        <h2 className="text-2xl font-bold mb-12 text-center">How DeepMindQ Thinks</h2>
        ...
        {/* Architecture */}
        <h2 className="text-2xl font-bold mb-8 text-center">Live Intelligence Pipeline</h2>
          ...
          <div className="space-y-1">
            {[
              ...
              <div key={item.stage} className="flex items-center gap-3">
                ...
                <span
                  className={`w-2 h-2 rounded-full ${item.status === 'ai' ? 'bg-purple-400' : item.status === 'fusion' ? 'bg-green-400' : item.status === 'persist' ? 'bg-blue-400' : 'bg-gray-600'}`}
                />
                <span className={item.color}>{item.name}</span>
                {item.status === 'ai' && (
                  <span className="text-xs text-purple-500/50 ml-auto">LLM</span>
                )}
                {item.status === 'fusion' && (
