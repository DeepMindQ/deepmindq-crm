#!/usr/bin/env python3
"""
Phase 2.1: Create calibration job runner cron route.
Reads IntelligenceFeedback → computes calibration data points → feeds to CalibrationEngine.
"""
import os

route_path = '/home/z/my-project/src/app/api/cron/calibration-runner/route.ts'
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const feedbacks = await (await import('@/lib/db')).then(m =>
      m.db.intelligenceFeedback.findMany({
        where: {
    ...
    for (const fb of feedbacks) {
      try {
        ...
        try {
          await (await import('@/lib/db')).then(m =>
            m.db.intelligenceFeedback.update({
              where: { id: fb.id },
