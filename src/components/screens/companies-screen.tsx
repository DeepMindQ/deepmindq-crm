...
export function CompaniesScreen() {
  ...
  const companies = useMemo(() => {
    ...
    return [...data.companies].sort((a: any, b: any) => {
              ...
              {isLoading
                ...
                : companies.map((c: any) => (
                      <tr key={c.id} className="table-row-hover border-b border-gray-50 transition-colors group">
                          ...
                          <td className="px-4 py-2.5 cursor-pointer" onClick={() => { setSelectedCompanyId(c.id); setActiveView('company-profile') }}>
                            <div className="flex items-center gap-3">
                              <div className="size-8 rounded-lg bg-gray-100 overflow-hidden shrink-0 flex items-center justify-center">
                                {c.domain ? (
                                  <Image src={`https://logo.clearbit.com/${c.domain}`} alt="" width={32} height={32} className="size-8 object-contain p-1" onError={e => { (e.target as HTMLImageElement).style.display = 'none'; (e.target as HTMLImageElement).parentElement!.innerHTML = `<span class="text-xs font-bold text-gray-400">${c.name?.charAt(0)}</span>` }} />
                                ) : (
                          ...
                          <td className="px-4 py-2.5">
                            <div className="flex items-center gap-2">
                              <StatusDot status={(c.dataFreshness as any) || 'unknown'} pulse />
                          ...
                          <td className="px-4 py-2.5">
                            <button
                              ...
                              className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium border transition-colors hover:opacity-80 ${statusStyle[c.status] || statusStyle.new}`}
                            >
                              {c.status}
                            </button>
                          </td>
