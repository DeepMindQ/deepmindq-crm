...
export type ApiHandler = (request: Request, context?: { params: Record<string, string> }) => Promise<Response>;
...
export function withErrorHandler(handler: ApiHandler): ApiHandler {
  return async (request, context) => {
    try {
      ...
      if (msg.includes('P2025') || msg.includes('Record to update not found') || msg.includes('Record to delete not found')) {
      ...
      if (msg.includes('P1001') || msg.includes('Can\'t reach database') || msg.includes('Connection')) {
      ...
      // Unexpected errors
      logger.error('[API Error Handler] Unhandled error', { error: msg, stack: error instanceof Error ? error.stack : undefined });
