...
type TimelineEvent = {
  ...
  title: string;
  description: string;
  type: 'email' | 'meeting' | 'call' | 'note' | 'milestone';
};
const activityTimeline: TimelineEvent[] = [
  {
    date: 'Jan 20',
    title: 'Pricing proposal sent',
    description: 'Sent revised enterprise pricing to CFO Sarah Chen',
    type: 'email',
  },
  {
    date: 'Jan 17',
    title: 'Technical deep-dive',
    description: '2hr session with VP Engineering on API integration',
    type: 'meeting',
  },
  {
    date: 'Jan 14',
    title: 'Competitive evaluation update',
    description: 'Acme confirmed we are in final 2 with VendorX',
    type: 'note',
  },
  {
    date: 'Jan 10',
    title: 'ROI presentation',
    description: 'Presented business case to C-suite leadership team',
    type: 'meeting',
  },
  {
    date: 'Jan 5',
    title: 'Discovery call completed',
    description: 'Initial requirements gathering with IT Director',
    type: 'call',
  },
  {
    date: 'Dec 28',
    title: 'Opportunity qualified',
    description: 'BANT criteria met, moved to pipeline',
    type: 'milestone',
  },
];
...
const aiCoachTips = [
  {
    id: '1',
    tip: 'Focus on CFO Chen — she controls budget sign-off. Schedule a 1:1 to address remaining pricing objections before the Feb board meeting.',
    priority: 'high' as const,
  },
  {
    id: '2',
    tip: 'James Rodriguez has been reviewing your API docs heavily. Send a technical case study from a similar FinTech implementation to reinforce confidence.',
    priority: 'medium' as const,
  },
  {
    id: '3',
    tip: 'Risk: VendorX recently lowered pricing by 15%. Prepare a value-comparison deck highlighting total cost of ownership advantages.',
    priority: 'high' as const,
  },
];
...
const relatedSignals = [
  ...
  {
    title: 'CFO Chen spoke at SaaS conference about platform consolidation',
    type: 'intent',
    time: '5d ago',
    confidence: 89,
  },
  {
    title: 'Acme renewing AWS contract — potential multi-cloud need',
    type: 'opportunity',
    time: '1w ago',
    confidence: 76,
  },
...
const competitiveIntel = [
  {
    ...
    note: 'Recently dropped pricing 15%. Strong executive sponsorship.',
  },
  {
    competitor: 'LegacySolution',
    status: 'Incumbent',
    threat: 'medium' as const,
    note: 'Current provider. Renewal in 6 months. Limited AI capabilities.',
  },
  {
    competitor: 'CloudNative Inc',
    status: 'Evaluated',
    threat: 'low' as const,
    note: 'Dropped from evaluation per IT Director. Missing compliance features.',
  },
];
      ...
      <Card className="overflow-hidden">
        ...
        >
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
              ...
              <div>
                ...
                >
                  <span className="flex items-center gap-1.5">
                    <DollarSign className="w-3.5 h-3.5" />${(opportunity.value / 1000).toFixed(0)}K
                  </span>
                  <span className="flex items-center gap-1.5">
            ...
            <div className="flex gap-2">
              ...
              <Button
                size="sm"
                className="gap-1.5"
                style={{ backgroundColor: tokens.accent.primary, color: '#fff' }}
              >
          ...
          <div className="mt-5">
            <div className="flex justify-between mb-2">
              {stageLabels.map((label, i) => (
                <div key={label} className="flex flex-col items-center gap-1">
                  <div
                    className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${i + 1 <= opportunity.stageOrder ? 'text-white' : ''}`}
                    style={{
                      backgroundColor:
                        i + 1 <= opportunity.stageOrder
                          ? tokens.accent.primary
                          : tokens.neutral['100'],
                      color: i + 1 > opportunity.stageOrder ? tokens.text.muted : '#fff',
                    }}
                  >
                    {i + 1 <= opportunity.stageOrder ? '✓' : i + 1}
                  </div>
                  <span
                    className="text-[10px] hidden sm:block max-w-[80px] text-center"
                    style={{
                      color:
                        i + 1 === opportunity.stageOrder ? tokens.text.primary : tokens.text.muted,
                    }}
                  >
                    {label}
                  </span>
            ...
            <Progress
              value={(opportunity.stageOrder / opportunity.totalStages) * 100}
        ...
        {keyMetrics.map((m) => (
          <Card key={m.label} className="py-4 gap-2">
            <CardContent className="p-4 pb-0">
              ...
              <p className="text-lg font-bold mt-1" style={{ color: tokens.text.primary }}>
                {m.value}
              </p>
              <p className="text-[11px] mt-1" style={{ color: tokens.text.secondary }}>
                {m.change}
              </p>
            </CardContent>
          </Card>
                ...
                <div key={i} className="flex gap-3 pb-4 relative">
                  ...
                  <div className="flex-1 min-w-0 pt-0.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] shrink-0" style={{ color: tokens.text.muted }}>
                        {event.date}
                      </span>
                    </div>
              ...
              {aiCoachTips.map((tip) => (
                <div
                  ...
                  style={{
                    backgroundColor:
                      tip.priority === 'high'
                        ? `${tokens.confidence.medium.bg}50`
                        : `${tokens.neutral['100']}50`,
                    border: `1px solid ${tip.priority === 'high' ? tokens.confidence.medium.border : tokens.borderFaint}`,
                  }}
                >
              ...
              {steps.map((step) => (
                ...
                >
                  ...
                  <span
                    ...
                    style={{ color: step.done ? tokens.text.muted : tokens.text.primary }}
                  >
                    {step.text}
                  </span>
                </label>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle
                ...
                style={{ color: tokens.text.primary }}
              >
                <Swords className="w-4 h-4" style={{ color: tokens.confidence.low.value }} />{' '}
                Competitive Intel
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle
                ...
                style={{ color: tokens.text.primary }}
              >
                <Target className="w-4 h-4" style={{ color: tokens.domain.opportunity }} /> Related
                Signals
              </CardTitle>
            </CardHeader>
            <CardContent>
                ...
                {relatedSignals.map((sig, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] mt-0.5" style={{ color: tokens.text.muted }}>
                        {sig.time} · {sig.confidence}% confidence
                      </p>
                    </div>
                  </div>
                ))}
