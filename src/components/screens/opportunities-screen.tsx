  ...
  const filtered = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(
        (o) => o.company.toLowerCase().includes(q) || o.dealName.toLowerCase().includes(q),
      );
    }
    // Sort
    result.sort((a, b) => {
  ...
  const columns = useMemo(
    () => [
      ...
      { key: 'dealName', label: 'Deal Name', sortable: true },
      {
        ...
        sortable: true,
        render: (v: unknown) => (
          <span className="font-semibold" style={{ color: tokens.confidence.high.value }}>
    ...
    >
      ...
      <Sheet open={!!selectedOpp} onOpenChange={(open) => !open && setSelectedOpp(null)}>
        <SheetContent
          ...
          style={{ background: 'var(--ios-bg-card)', borderLeft: `1px solid ${border}` }}
        >
          {selectedOpp && (
            <>
              <SheetHeader>
                <SheetTitle className="text-lg font-bold" style={{ color: textPrimary }}>
                  {selectedOpp.dealName}
                </SheetTitle>
              </SheetHeader>
              ...
              <div className="mt-6 space-y-6">
                ...
                >
                  ...
                  <p className="text-2xl font-bold" style={{ color: tokens.confidence.high.value }}>
                    {formatCurrency(selectedOpp.value)}
                  </p>
                  <p className="text-xs mt-1" style={{ color: textSecondary }}>
                    Weighted: {formatCurrency(selectedOpp.value * (selectedOpp.probability / 100))}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    {
                      label: 'Close Date',
                      value: new Date(selectedOpp.closeDate).toLocaleDateString('en-US', {
                        month: 'long',
                        day: 'numeric',
                        year: 'numeric',
                      }),
                    ...
                    {
                      label: 'Created',
                      value: new Date(selectedOpp.createdAt).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric',
                      }),
                    ...
                    {
                      label: 'Days Active',
                      value: `${daysBetween(selectedOpp.createdAt, new Date().toISOString().split('T')[0])} days`,
                      icon: Building2,
                    },
                  ].map((item) => (
                    <div
