...
export interface ListCompaniesParams {
  ...
  tier?: string
  sizeRange?: string
  sortBy?: 'accountPriorityScore' | 'intelligenceScore' | 'opportunityScore' | 'contacts' | 'signals' | 'lastActivityAt' | 'updatedAt'
  sortOrder?: 'asc' | 'desc'
  page?: number
  limit?: number
  cursor?: string
...
export interface BatchExecuteInput {
  action: 'archive' | 'restore' | 'delete' | 'enrich' | 'reassign' | 'export' | 'tag' | 'update_status'
...
export class DeepMindQClient {
   ...
   */
  async login(email: string, password: string): Promise<{ success: boolean; message: string; devCode?: string }> {
  ...
  async listCompanies(params?: ListCompaniesParams): Promise<{
    ...
    filters?: { tiers: Array<{ tier: string; count: number }>; statuses: Array<{ status: string; count: number }> }
  }> {
  ...
  async getCompanySignals(id: string, params?: { type?: string; severity?: string }): Promise<{ signals: any[] }> {
  ...
  async testWebhook(data: TestWebhookInput): Promise<{ success: boolean; statusCode?: number; responseTime?: number }> {
